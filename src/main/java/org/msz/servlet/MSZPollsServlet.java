...
public class MSZPollsServlet extends HttpServlet
{
  private Logger logger;
  MSZPollsServletDAO dao;
  WebUtils webUtils;
  static Utils utils;
      ...
      throws Exception
  {
    ...
    {
      String message = readURL(poll.id);
  ...
  private String readURL(int pollID) throws Exception
  {
    String url = getPollURL(pollID);
    String message;
    try
    {
  ...
  public static String getPollURL(int pollID) throws Exception
  {
    return servletConfig.getProperty("servlet.url") + "/inputVote.jsp?pollID="
        + pollID;
  }
    ...
    if (userID == null)
    {
      ...
      else
      {
        ...
        sendEmail(user.id,
            ...
            emailBody.replaceAll("__USERID__", String.valueOf(user.id)).replaceAll("__URL__", servletConfig.getProperty("servlet.url"))
            );
