...
public class WebUtils
{
  ...
  public WebUtils(Logger logger)
  {
    this.logger = logger;
  }

  public Long convertToDate(String parameter, String pattern) throws Exception
  {
    ...
    {
      logger.error("WebUtils.convertToDate(): ", e);
      throw new Exception(
          "Rossz d&aacute;tum form&aacute;tum! <b>yyyy-MM-dd</b>");
    }
  }

  public static String convertToString(long date, String pattern)
      throws Exception
  {
    SimpleDateFormat format = new SimpleDateFormat(pattern);
  ...
  public static String getOptionalParameter(HttpServletRequest request,
      String parameter)
  {
    try
    {
      ...
      throws Exception
  {
    ...
    if (value == null)
    {
      ...
      if (session != null)
      {
        ...
        logDebug("WebUtils.getParameter(): read parameter: " + parameter
            + " from HttpSession. value: " + value);
      }

      if (value == null)
      {
        throw new Exception(parameter
            + " is not found in HTTP request or in HTTP session! HTTP keys: "
      ...
      logDebug("WebUtils.getParameter(): read parameter: " + parameter
          + " from HttpServletRequest. value: " + value);

    // logDebug("WebUtils.getParameter(): value: " + new
    // String(value.getBytes("UTF-8"), "UTF-8") + " " + encodeString(new
    // String(value.getBytes("UTF-8"), "UTF-8")));
    // logDebug("WebUtils.getParameter(): value: " + new
    // String(value.getBytes("ISO-8859-1"), "UTF-8") + " " + encodeString(new
