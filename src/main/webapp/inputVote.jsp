...
<%
  String pollURL = MSZPollsServlet.getPollURL(pollID);
%>
...
<p>
<form accept-charset="UTF-8" name="input" action="MSZPollsServlet"
	method="post"><input type="hidden"
	name="<%=MSZPollsServlet.COMMAND%>"
	value="<%=MSZPollsServlet.COMMAND_SAVE_VOTE%>"> <input
	type="hidden" name="<%=MSZPollsServlet.POLL_ID%>" value="<%=pollID%>">

<%
  if (session.getAttribute(MSZPollsServlet.USER_ID) == null)
  {
%> Szavaz&oacute; email c&iacute;me: <input type="text"
	name="<%=MSZPollsServlet.ANONYMOUS_VOTE%>">
<p>
<%
  }
%>
 ...
 %>
		...
		<%
		    if (poll instanceof SingleDecisionPoll)
		    {
		      if (httpIndex == 1)
		        %> 
		        <input type='hidden' name='<%=MSZPollsServlet.OPTION_VALUE + 1 %>' value='igen' >

				<td>
                    <%=option.name %>
                </td>
                <td>
 		             <input type='radio' name='<%=MSZPollsServlet.OPTION_NAME + 1 %>' value='<%=option.name %>' >
                </td>
		        <%
		    }
		    ...
		    {
		      %>
		      <input type='hidden' name='<%=MSZPollsServlet.OPTION_NAME + httpIndex %>' value='<%=option.name %>' >
                <td>
                    <%=option.name %>
                </td>
                <td>
              <input type='checkbox' name='<%=MSZPollsServlet.OPTION_VALUE + httpIndex %>' value='igen' >
                </td>
              <%
		    }
		    else if (poll instanceof SplitPointsPoll)
		    {
		      %>
                ...
                <td>
                   <input type='hidden' name='<%=MSZPollsServlet.OPTION_NAME + httpIndex %>' value='<%=option.name %>' >
	       	       <input type='text' name='<%=MSZPollsServlet.OPTION_VALUE + httpIndex %>' id='<%=MSZPollsServlet.OPTION_VALUE + httpIndex %>'  onChange='recalculateFreeAmount()'>
                </td>
            <%
		    }
		...
		%> 
                ...
                <td>
<input type='hidden'
			name='<%=MSZPollsServlet.OPTION_NAME + httpIndex%>'
			value='<%=option.name%>'> 
			<%
   for (int i = 1; i <= awardingPoll.maxAmount; i++)
       {
 %> <input type='radio'
			name='<%=MSZPollsServlet.OPTION_VALUE + httpIndex%>' value='<%=i%>'>
		<%=i%> <%
   }
 %>                 
</td>
...
<input type="text"
			name="<%=MSZPollsServlet.OPTION_VALUE + httpIndex%>"
...
<input type="button" value="^"
			onClick="moveUp(this.parentNode)"> 
<input type="button"
			value="v" onClick="moveDown(this.parentNode)"></div>
		<%
		  }
...
<input type="hidden"
			...
			value="<%=option.name%>"> 
			<%
   if (MSZPollsServlet.OPTION_TYPE_TEXT.equals(option.type))
       {
 %> <input type="text"
			name="<%=MSZPollsServlet.OPTION_VALUE + httpIndex%>"> <br>
		<%
		  }
		      else if (MSZPollsServlet.OPTION_TYPE_CHECKBOX.equals(option.type))
		      {
		%> <input type="checkbox"
			name="<%=MSZPollsServlet.OPTION_VALUE + httpIndex%>"  value='igen'> <br>
		<%
		  }
		      else if (MSZPollsServlet.OPTION_TYPE_RADIO.equals(option.type))
		      {
