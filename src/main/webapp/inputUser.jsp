...
<%@page import="org.msz.servlet.util.MSZPollsServletDAO"%><html>
...
<input type="hidden" name="<%=MSZPollsServlet.COMMAND%>"
	value="<%=MSZPollsServlet.COMMAND_SAVE_USER%>">

<table border="0">
	<tr>
		<td>Email c&iacute;m:</td>
		<td><input type="email"
			name="<%=MSZPollsServlet.USER_EMAIL_ADDRESS%>"
	...
	<tr>
		...
		<td><input type="password"
			name="<%=MSZPollsServlet.USER_PASSWORD%>"
	...
	<tr>
		...
		<p>C&iacute;m: <input type="text" size="60"
			name="<%=MSZPollsServlet.USER_ADDRESS%>"
		...
		<input type="hidden" name="<%=MSZPollsServlet.USER_ADDRESS_LAT%>"
			value="<%=user != null ? user.lat : ""%>" id="pointLat" /> <input
			type="hidden" name="<%=MSZPollsServlet.USER_ADDRESS_LNG%>"
