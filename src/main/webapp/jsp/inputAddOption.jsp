...
function addRow()
{
	...
	newRow.insertCell(-1).innerHTML='<input name="<%=PollsServlet.OPTION_NAME %>'+document.input.rows.value+'" type="text">';
}
...
<body>
<form name="input" action="../PollsServlet" method="post"><input
	type="hidden" name="<%=PollsServlet.COMMAND %>"
	value="<%=PollsServlet.COMMAND_ADD_OPTION %>"> <input
	type="hidden" name="rows" value="0"> <%
     ...
     Poll poll = (Poll) dao.get(Integer.parseInt((String) session
 		    .getAttribute(PollsServlet.POLL_ID)), Poll.class);
 %>
...
<table width="77%" border="1" cellspacing="0" cellpadding="0">
	...
	document.input.rows.value = 0;
		addRow();
</script> <input name="button" type="button" onClick="addRow()"
							value="&Uacute;j sor">
						</td>
					</tr>
				</table>
