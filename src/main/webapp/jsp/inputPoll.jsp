...
function addRow()
{
	...
	newRow.insertCell(-1).innerHTML='<input name="<%=PollsServlet.OPTION_NAME%>' + document.getElementById('form1').rows.value + '" type="text">';
	...
	{
		newRow.insertCell(-1).innerHTML="T&iacute;pusa: <select name=\"<%=PollsServlet.OPTION_TYPE%>"+document.getElementById('form1').rows.value+"\"> \
    <option value=\"<%=PollsServlet.OPTION_TYPE_RADIO%>\" onClick=\"document.getElementById(\'div' + document.getElementById('form1').rows.value + '\').style.visibility = \'visible\'\">R&aacute;dio</option> \
    <option value=\"<%=PollsServlet.OPTION_TYPE_CHECKBOX%>\" onClick=\"document.getElementById(\'div' + document.getElementById('form1').rows.value + '\').style.visibility = \'hidden\'\">Kipip&aacute;l&oacute;s</option> \
    <option value=\"<%=PollsServlet.OPTION_TYPE_TEXT%>\" onClick=\"document.getElementById(\'div' + document.getElementById('form1').rows.value + '\').style.visibility = \'hidden\'\">Sz&ouml;vegmez&ocirc;</option> \
  </select>";

	newRow.insertCell(-1).innerHTML='<div id=\"div' + document.getElementById('form1').rows.value + '\">A lehet&ocirc;s&eacute;gek vessz&ocirc;vel elv&aacute;lasztva <input name="<%=PollsServlet.OPTION_VALUE%>' + document.getElementById('form1').rows.value + '" type="text"><div>';
	}
...
%><form accept-charset="UTF-8" id="form1" action="../PollsServlet"
	method="post">
<input type="hidden"
	name="<%=PollsServlet.COMMAND%>"
id="commandID"
	value="<%=PollsServlet.COMMAND_SAVE_POLL%>"> <%
   if (pollID != null)
     out.println("<input type='hidden' name='" + PollsServlet.POLL_ID
         + "' value='" + pollID + "'>");
		...
		<tr>
			...
			<table border="0" width="100%">
				<tbody>
					<tr>
						...
						<td width="512"><input name="<%=PollsServlet.POLL_TITLE%>"
							value="<%=poll == null ? "" : poll.title%>" type="text" size="50"></td>
					</tr>
					<tr bgcolor="#dddddd">
						<td>Szavazás leírása:</td>
						<td><textarea name="<%=PollsServlet.POLL_DESCRIPTION%>"
					...
					<tr>
						...
						<td><img
							src="./images/cal.gif" border="0"> <input
							name="<%=PollsServlet.POLL_ENDDATE%>" type="date" id="endData"
							size="11"
							value="<%=poll == null ? "" : poll.endDate == Long.MAX_VALUE ? ""
          : WebUtils
              .convertToString(poll.endDate, PollsServlet.DATE_PATTERN)%>">
					...
					<tr bgcolor="#dddddd">
						...
						<td><input type="checkbox"
							name="<%=PollsServlet.POLL_USER_CAN_RESUBMIT%>"
							<%=poll == null ? ""
          : poll.userCanResubmit ? "checked='checked'" : ""%>></td>
					...
					<tr>
						...
						<td><input type="checkbox"
							name="<%=PollsServlet.POLL_USER_CAN_ADD_ENTRY%>"
							<%=poll == null ? ""
          : poll.userCanAddEntry ? "checked='checked'" : ""%>></td>
					</tr>
					<tr bgcolor="#dddddd">
						<td>Szavazás típusa:</td>
						<td><select id="<%=PollsServlet.POLL_CLASS%>"
							name="<%=PollsServlet.POLL_CLASS%>" size="5">
          ...
          : dao.getPublicPoll(poll.id) != null ? "checked='checked'" : ""%>></td>
                ...
                <td>Mértékegység:</td>
						<td><input name="<%=PollsServlet.UNITS%>" type="text"
							value="<%=poll == null ? ""
          : poll instanceof SplitPointsPoll ? ((SplitPointsPoll) poll).units
              : ""%>"></td>
                ...
                <td>Szétosztható Mennyiség:</td>
						<td><input name="<%=PollsServlet.MAX_AMOUNT%>" type="text"
							value="<%=poll == null ? ""
						...
						<table width="100%" border="1" cellspacing="0" cellpadding="0">
							<tr>
								...
								%> <%=option.name%> <input type="checkbox"
									name="<%=PollsServlet.POLL_OPTION_ID%>"
									value="<%=option.id%>">
<input type="submit" value="T&ouml;rl&eacute;s" onClick="document.getElementById('commandID').value='<%=PollsServlet.COMMAND_DELETE_POLL_OPTION %>'">
 <br>
								<%
								  }
								  }
